...
export default function TimetablePage() {
  ...
  const [userChangedStructure, setUserChangedStructure] = useState(false)
  ...
  // When section changes, auto-set structure to section's default (unless user manually changed it)
  useMemo(() => {
    if (selectedSection?.structureId && !userChangedStructure) {
      setSelectedStructureId(selectedSection.structureId)
    } else if (!selectedSection?.structureId && !userChangedStructure) {
      setSelectedStructureId("")
    }
  }, [selectedSection, userChangedStructure])

  // Resolve the effective structureId: user-selected overrides section default
  ...
  // Get the selected structure name for display
  const selectedStructure = useMemo(() => {
    return structures.find((s: any) => s.id === effectiveStructureId)
  }, [structures, effectiveStructureId])
          ...
          <CardContent className="pt-6">
                ...
                <HierarchicalFilter
                  ...
                  onChange={(values) => {
                    ...
                    if (values.sectionId !== undefined) {
                      ...
                      // Reset user structure override when section changes
                      setUserChangedStructure(false)
                    }
                  }}
                  labels={{
                    courseId: "Course",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={selectedSectionId ? "Select structure" : "Select a section first"} />
                  </SelectTrigger>
                  <SelectContent>
                    {structures.map((s: any) => (
                      <SelectItem key={s.id} value={s.id}>
                        {s.name}
                        {selectedSection?.structureId === s.id ? " (Section default)" : ""}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {selectedStructure && (
                  <p className="text-xs text-muted-foreground">
                    Structure: <span className="font-medium">{selectedStructure.name}</span>
                    {selectedSection?.structureId === effectiveStructureId && selectedSection?.structureId
                      ? " (from section)"
                      : userChangedStructure
                        ? " (manually selected)"
                        : ""}
                  </p>
                )}
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-12 text-center text-muted-foreground">
                      {!effectiveStructureId
                        ? "No timetable structure selected. Select a structure above or assign one to the section."
                        : "No periods found for this structure. Create periods in the timetable structure first."}
                    </td>
                  </tr>
                ) : (
