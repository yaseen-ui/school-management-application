  ...
  return {
    ...
    students: row.students
      ? row.students.map((sp) => {
          ...
          // Prefer active academic year enrollment, else most recent active enrollment
          const enrollments = student.enrollments || [];
          const currentEnrollment =
            enrollments.find((e) => e.academicYear?.status === "active") ||
            enrollments[0] ||
            null;
          return {
            id: student.id,
            ...
            gradeName:
              currentEnrollment?.grade?.gradeName || student.grade?.gradeName,
            sectionName:
              currentEnrollment?.section?.sectionName || student.section?.sectionName,
            isPrimary: sp.isPrimary,
            enrollmentId: currentEnrollment?.id || null,
            academicYearLabel: currentEnrollment?.academicYear?.name || null,
          };
        })
      : [],
...
export default {
  ...
  async registerParent({ token, email, phone, password }) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Assign the "Parent" role to the newly created user
      const parentRole = await tx.role.findUnique({
        where: { uniqueRoleNamePerTenant: { tenantId: parent.tenantId, roleName: "Parent" } },
        select: { id: true },
      });
  ...
  // Align with student-dashboard:
  //  - released admin exams (not draft/cancelled/archived)
  //  - custom section schedules with no parent exam (e.g. Slip Test)
  async getMyChildrenResults(userId, tenantId, { examId }) {
        ...
        students: {
          include: {
            student: {
              include: {
                enrollments: {
                  // StudentEnrollment has no deletedAt — filter by tenant + active
                  where: { tenantId, status: "active" },
                  select: { id: true, sectionId: true },
                },
                grade: true,
    ...
    // Fetch marks: released admin exams OR released custom section schedules
    const marksWhere = {
      tenantId,
      enrollmentId: { in: enrollmentIds },
      examPaper: {
        schedule: {
          OR: [
            {
              exam: {
                status: { in: RELEASED_EXAM_STATUSES },
              },
            },
            {
              examId: null,
          ...
          include: {
            schedule: {
              select: {
                ...
                examId: true,
                status: true,
                exam: { select: { id: true, name: true, examType: true, status: true } },
              },
            },
            sectionSubject: {
              include: {
    ...
    // Filter by examId if provided (supports admin exam id or custom:scheduleId)
    const filteredMarks = examId
      ? marks.filter((m) => {
          const schedule = m.examPaper?.schedule
          const exam = schedule?.exam
          if (exam?.id === examId) return true
          if (!exam && schedule?.id) {
            return schedule.id === examId || examId === `custom:${schedule.id}`
          }
          return false
        })
      : marks
    ...
    // Custom schedules (no parent exam) get their own group keyed by schedule id.
    ...
    for (const mark of filteredMarks) {
      ...
      const examKey = exam?.id || `custom:${schedule.id}`
      if (!examMap.has(examKey)) {
        examMap.set(examKey, {
          examId: exam?.id || `custom:${schedule.id}`,
