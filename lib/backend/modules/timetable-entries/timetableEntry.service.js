...
class TimetableEntryService {
  async #validateConflicts(data, tenantId, excludeEntryId = null) {
    const { academicYearId, dayOfWeek, periodId, teacherAssignmentId, room } = data;
    if (!academicYearId || dayOfWeek === undefined || dayOfWeek === null || !periodId) return;

    // Build base where clause for conflict checks (exclude self when updating)
    const baseWhere = {
      tenantId,
      academicYearId,
      dayOfWeek: mapDayOfWeekToEnum(dayOfWeek),
      periodId,
    };
    if (excludeEntryId) {
      baseWhere.id = { not: excludeEntryId };
    }

    // 1. Teacher availability check
    if (teacherAssignmentId) {
      // Fetch the teacher assignment to get teacherId
      const assignment = await prisma.teacherAssignment.findFirst({
        where: { id: teacherAssignmentId, tenantId },
      ...
      // Fetch the period to get startTime/endTime
      ...
      // Check teacher availability for this day + time range
      ...
      if (!availableSlot) {
        throw new Error(
          "Teacher is not available during this time slot. Please check the teacher's availability schedule."
        );
      }

      // 2. Teacher double-booking check
      const conflictingTeacherEntry = await prisma.timetableEntry.findFirst({
        where: {
          ...baseWhere,
          teacherAssignment: {
            teacherId: assignment.teacherId,
      ...
      if (conflictingTeacherEntry) {
        const sectionName = conflictingTeacherEntry.sectionSubject?.section?.sectionName || "another section";
        throw new Error(
          `Teacher is already assigned to ${sectionName} during this period.`
        );
      }
    }
    ...
    if (room) {
      const conflictingRoomEntry = await prisma.timetableEntry.findFirst({
        where: {
          ...baseWhere,
          room,
        },
      ...
      if (conflictingRoomEntry) {
        const sectionName = conflictingRoomEntry.sectionSubject?.section?.sectionName || "another section";
        throw new Error(
          `Room "${room}" is already booked for ${sectionName} during this period.`
        );
      }
    }
  }
  ...
  async create(data, tenantId) {
    ...
    // Validate conflicts (teacher availability, teacher double-booking, room conflict)
    await this.#validateConflicts(toCreate, tenantId);
  ...
  async update(id, data, tenantId) {
    ...
    // Merge existing data with updates for conflict validation
    const mergedData = {
      academicYearId: existing.academicYearId,
      dayOfWeek: toUpdate.dayOfWeek ?? existing.dayOfWeek,
      periodId: toUpdate.periodId ?? existing.periodId,
      teacherAssignmentId: toUpdate.teacherAssignmentId ?? existing.teacherAssignmentId,
      room: toUpdate.room !== undefined ? toUpdate.room : existing.room,
    };

    // Validate conflicts (teacher availability, teacher double-booking, room conflict)
    await this.#validateConflicts(mergedData, tenantId, id);
