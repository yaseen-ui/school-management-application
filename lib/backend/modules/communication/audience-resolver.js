...
export class AudienceResolver {
  ...
  async resolve(criteria, tenantId) {
    ...
    if (criteria.targetRoles?.length) {
      ...
      const userRoles = await prisma.userRole.findMany({
        ...
        select: { userId: true, user: { select: { id: true, email: true, phone: true, fullName: true } } },
      });
      const map = new Map();
    ...
    if (criteria.targetEmployeeTypes?.length) {
      const teachers = await prisma.teacher.findMany({
        ...
        select: { user: { select: { id: true, email: true, phone: true, fullName: true } } },
      });
      const map = new Map();
    ...
    return this.#enrichContacts([...merged.values()], tenantId);
  }
  ...
  async #resolveAllUsers(tenantId) {
    ...
    return this.#enrichContacts(users, tenantId);
  }

  /**
   * Enrich user contacts with fullName and fallback phones from Parent / Teacher records.
   * WhatsApp/SMS often need parent.phone when user.phone is empty.
   */
  async #enrichContacts(users, tenantId) {
    ...
    const [parents, teachers] = await Promise.all([
      prisma.parent.findMany({
        ...
        select: { userId: true, phone: true, fullName: true, email: true },
      }),
    ...
    const parentByUser = new Map(parents.map((p) => [p.userId, p]));
    const teacherByUser = new Map(teachers.map((t) => [t.userId, t]));

    return users.map((u) => {
      const parent = parentByUser.get(u.id);
      const teacher = teacherByUser.get(u.id);
      return {
        userId: u.id,
        email: u.email || parent?.email || teacher?.email || null,
        phone: u.phone || parent?.phone || teacher?.phone || null,
        fullName: u.fullName || parent?.fullName || teacher?.fullName || null,
      };
    });
  }
}