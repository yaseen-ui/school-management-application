...
class CommunicationController {
  ...
  /**
   * GET /api/communications/search/users — Search users by name or email.
   */
  static async searchUsers(req, res) {
    try {
      ...
      return responseHandler(res, "success", users, "Users retrieved successfully.");
    } catch (error) {
  ...
  /**
   * GET /api/communications/search/roles — Search roles by name.
   */
  static async searchRoles(req, res) {
    try {
      ...
      const roles = await prisma.role.findMany({
        where: {
          tenantId: req.tenantId,
          ...(q ? { roleName: { contains: q, mode: "insensitive" } } : {}),
        },
        select: { id: true, roleName: true },
        take: limit ? parseInt(limit) : 20,
        orderBy: { roleName: "asc" },
      });
      return responseHandler(res, "success", roles, "Roles retrieved successfully.");
    } catch (error) {
  ...
  /**
   * GET /api/communications/search/grades — Search grades by name.
   */
  static async searchGrades(req, res) {
    try {
      const { q, limit } = req.query;
      const { prisma } = await import("../../lib/prisma.js");
      const grades = await prisma.grade.findMany({
        where: {
          tenantId: req.tenantId,
          ...(q ? { gradeName: { contains: q, mode: "insensitive" } } : {}),
        },
        select: { id: true, gradeName: true, course: { select: { courseName: true } } },
        take: limit ? parseInt(limit) : 20,
        orderBy: { gradeName: "asc" },
      });
      return responseHandler(res, "success", grades, "Grades retrieved successfully.");
    } catch (error) {
      logger.error(`Error searching grades: ${error.message}`);
      return responseHandler(res, "fail", null, error.message);
    }
  }

  /**
   * GET /api/communications/search/sections — Search sections by name.
   */
  static async searchSections(req, res) {
    try {
      const { q, limit } = req.query;
      const { prisma } = await import("../../lib/prisma.js");
      const sections = await prisma.section.findMany({
        where: {
          tenantId: req.tenantId,
          ...(q ? { sectionName: { contains: q, mode: "insensitive" } } : {}),
        },
        select: { id: true, sectionName: true, grade: { select: { gradeName: true } } },
        take: limit ? parseInt(limit) : 20,
        orderBy: { sectionName: "asc" },
      });
      return responseHandler(res, "success", sections, "Sections retrieved successfully.");
    } catch (error) {
      logger.error(`Error searching sections: ${error.message}`);
      return responseHandler(res, "fail", null, error.message);
    }
  }
