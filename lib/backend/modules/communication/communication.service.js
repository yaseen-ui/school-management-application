...
/** Modest concurrency for external channel APIs (WhatsApp rate limits). */
const DISPATCH_CONCURRENCY = Number(process.env.COMM_DISPATCH_CONCURRENCY || 5);
...
export class CommunicationService {
   ...
   * @param {Array}  [params.channels]     — ["in_app", "email", "sms", "push", "whatsapp"]
   ...
   * @param {string} [params.whatsappTemplate] — registry key or Meta template name
   * @param {Object} [params.templateData]     — extra vars for WhatsApp template params
  ...
  async send(params, tenantId, actorId) {
    ...
    // 5. Create recipient rows + dispatch (bounded concurrency for external APIs)
    let delivered = 0;
    let failed = 0;
    let skipped = 0;
    ...
    for (const user of resolvedUsers) {
      for (const channel of enabledChannels) {
        jobs.push({ user, channel });
      }
    }

    const dispatchOne = async ({ user, channel }) => {
      try {
    ...
    await this.#mapPool(jobs, DISPATCH_CONCURRENCY, dispatchOne);
