...
class SectionsService {
  ...
  async updateSection(sectionId, data, tenantId) {
    ...
    const { sectionName, gradeId, roomId, sectionInChargeId } = data || {};
    if (!gradeId) {
      throw new Error("gradeId is required to update a section.");
    }

    const targetGradeId = gradeId || existing.gradeId;
    if (
