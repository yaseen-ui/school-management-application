...
class AttendanceService {
    ...
    const mark = await prisma.attendanceMark.upsert({
      where: {
        tenantId_sessionId_enrollmentId: {
          tenantId: toUpsert.tenantId,
          sessionId: toUpsert.sessionId,
          enrollmentId: toUpsert.enrollmentId,
        },
      },
      update: { status: toUpsert.status, remarks: toUpsert.remarks },
      create: toUpsert,
    });
  ...
  async getStudentAttendanceDetail(tenantId, enrollmentId, month, year) {
    ...
    // Deduplicate: keep only the latest mark per (sessionId) combination
    // This handles pre-existing duplicates that may have accumulated before the upsert fix
    const seen = new Set();
    const deduped = [];
    for (const mark of marks) {
      if (!seen.has(mark.sessionId)) {
        seen.add(mark.sessionId);
        deduped.push(mark);
      }
    }

    const total = deduped.length;
    const present = deduped.filter((m) => m.status === "present").length;
    const absent = deduped.filter((m) => m.status === "absent").length;
