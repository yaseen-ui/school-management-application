...
// ─── Helper: Resolve or create "Student Fees" account category ───────────────

const STUDENT_FEES_CATEGORY_NAME = "Student Fees";

async function resolveStudentFeesCategory(tenantId, tx) {
  // Try to find an existing "Student Fees" category
  let category = await tx.accountCategory.findFirst({
    where: { tenantId, name: STUDENT_FEES_CATEGORY_NAME },
  });
  ...
  if (!category) {
    category = await tx.accountCategory.create({
      data: {
        tenantId,
        name: STUDENT_FEES_CATEGORY_NAME,
        description: "Auto-created category for student fee payments",
        sortOrder: 0,
        isActive: true,
      },
    });
...
async function createPayment(data, tenantId) {
  ...
  // Use a Prisma interactive transaction so that if the account transaction
  // creation fails, the fee payment is rolled back as well.
  const result = await prisma.$transaction(async (tx) => {
    // 1. Create the fee payment
    ...
    // 2. Resolve or auto-create the "Student Fees" account category
    const category = await resolveStudentFeesCategory(tenantId, tx);

    // 3. Create a credit transaction in the accounts ledger
    await tx.accountTransaction.create({
      data: {
        ...
        transactionDate: payload.paymentDate || new Date(),
...
async function createRefund(data, tenantId) {
  ...
  // Use a Prisma interactive transaction so that if the account transaction
  // creation fails, the refund is rolled back as well.
  const result = await prisma.$transaction(async (tx) => {
    // 1. Create the fee refund
    ...
    // 3. Create a debit transaction in the accounts ledger (money going out)
    await tx.accountTransaction.create({
      data: {
        tenantId,
        categoryId: category.id,
        ...
        description: `Fee refund issued`,
        transactionDate: payload.refundDate || new Date(),
