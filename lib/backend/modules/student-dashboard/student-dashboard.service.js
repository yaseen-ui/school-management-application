 ...
 * Exams: released to parents once the exam leaves draft
 * (published / in_progress / completed / locked). Draft, cancelled, archived stay hidden.
 */

import { prisma } from "../../lib/prisma.js";
import { hasPermission, resolveEffectivePermissions } from "../../rbac/engine.js";

// Parent-visible exam statuses. Draft is staff-only until status is advanced.
const RELEASED_EXAM_STATUSES = ["published", "in_progress", "completed", "locked"];
// Custom section schedules (no parent exam row) — visible when schedule is released.
const RELEASED_CUSTOM_SCHEDULE_STATUSES = ["completed", "scheduled"];
...
export default {
  async getDashboard(enrollmentId, tenantId, user, { examScheduleId } = {}) {
    ...
    // Include:
    //  1) Marks under admin exams with a released status
    //  2) Marks under custom section schedules (examId null) e.g. Slip Test
        ...
        examPaper: {
          schedule: {
            OR: [
              ...
              {
                examId: null,
                status: { in: RELEASED_CUSTOM_SCHEDULE_STATUSES },
              },
            ],
          },
        },
    ...
    // Group by schedule (admin exam schedules + custom section schedules)
    const scheduleMap = new Map();
    for (const mark of marks) {
      const schedule = mark.examPaper?.schedule;
      if (!schedule) continue;
      const exam = schedule.exam; // null for custom section exams
      if (!scheduleMap.has(schedule.id)) {
        scheduleMap.set(schedule.id, {
          examId: exam?.id || null,
          examName: exam?.name || schedule.name,
          examType: exam?.examType || "custom",
          isCustom: !exam,
          scheduleId: schedule.id,
          scheduleName: schedule.name,
          startDate: exam?.startDate || schedule.startDate || null,
          endDate: exam?.endDate || schedule.endDate || null,
          marks: [],
        });
      }
    ...
    if (selectedScheduleId && scheduleMap.has(selectedScheduleId)) {
      ...
      // All section peer marks for these papers (class avg + ranking)
      const peerMarks =
        paperIds.length > 0
      ...
      for (const pm of peerMarks) {
        if (!pm.isAbsent && pm.marksObtained != null) {
          ...
          if (!totalsByEnrollment.has(pm.enrollmentId)) {
            totalsByEnrollment.set(pm.enrollmentId, 0);
          }
          totalsByEnrollment.set(
            pm.enrollmentId,
            totalsByEnrollment.get(pm.enrollmentId) + Number(pm.marksObtained)
          );
        }
      }

      // Section rank by total marks (ties share rank, same as staff results)
      const rankedPeers = [...totalsByEnrollment.entries()]
        .map(([id, peerTotal]) => ({ enrollmentId: id, total: peerTotal }))
        .sort((a, b) => b.total - a.total);

      let currentRank = 0;
      let previousTotal = null;
      const rankByEnrollment = new Map();
      rankedPeers.forEach((row, index) => {
        if (row.total !== previousTotal) {
          currentRank = index + 1;
          previousTotal = row.total;
        }
        rankByEnrollment.set(row.enrollmentId, currentRank);
      });

      const rank = rankByEnrollment.get(enrollmentId) ?? null;
      const cohortSize = rankedPeers.length;
