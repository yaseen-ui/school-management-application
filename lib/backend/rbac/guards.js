...
class Guard {
  ...
  static async action(req, permissionCode) {
    ...
    // #13: Company users cannot use tenant school APIs (even with admin:super).
    // Platform routes only (tenants, company users, auth, …).
    try {
      assertCompanyNotOnTenantApi(user, getRequestPathname(req));
    } catch (e) {
      if (e instanceof CompanyTenantForbiddenError || e?.code === 'COMPANY_TENANT_FORBIDDEN') {
        await auditAccessDenied(req, {
          permissionCode,
          reason: 'company_tenant_forbidden',
        });
        throw new ForbiddenError(e.message, 'COMPANY_TENANT_FORBIDDEN');
      }
      throw e;
    }

    // Company users on platform APIs bypass tenant RBAC (implicit platform access)
